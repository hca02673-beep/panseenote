...
(function () {
  ...
  function lockTableWidth() {
    ...
    // col 要素にインラインスタイルで幅を固定（CSS より優先されるため Chrome の再計算を上書き）
    var colTitle = table.querySelector("col.col-title");
    var colDate  = table.querySelector("col.col-date");
    var titleTd  = table.querySelector("td.col-title");
    if (colTitle && titleTd && titleTd.offsetWidth > 0) {
      colTitle.style.width = titleTd.offsetWidth + "px";
    }
    if (colDate) colDate.style.width = "0px";
  ...
  function unlockTableWidth() {
    ...
    if (colDate)  colDate.style.width  = "";
